...
RSpec.describe Ruson::Converter do
  ...
  it 'convert object to hash' do
    ...
    obj.name = 'ruby'
    expect(obj.to_hash).to eq({ id: nil, name: 'ruby', url: nil })
  end

  it 'convert nested object to hash' do
    obj = Post.new(post_json)
    obj.tags.first.name = 'Perl'
    expect(obj.to_hash).to eq({ id: 1, title: 'Ruson', user: { id: 1, name: 'klriutsa', url: 'https://github.com/klriutsa/ruson' }, content: 'Json ORM for Ruby Object', picture: { id: 1, title: 'Ruson Picture', url: 'https://picsum.photos/200' }, tags: [{ id: 1, name: 'Perl' }, { id: 2, name: 'Json' }], status: :published })
  end
